...
public class MainController {
	...
	public static void main(String []args) throws XMLStreamException, ParserConfigurationException, SAXException, IOException
	{
		...
		AnimalRepository a = new AnimalRepository("Animals.xml",Constants.XML_TAGS.ANIMAL);
		AnimalRepository b = new AnimalRepository("Animals.xml",Constants.XML_TAGS.ANIMAL);
		ArrayList<Animal> animalList= new ArrayList<Animal>();
		ArrayList<Animal> animalListLoaded= new ArrayList<Animal>();
		animalList.add(animal[0]);
		animalList.add(animal[1]);
		animalList.add(animal[2]);
		a.save(animalList);
		animalListLoaded=b.load();
		System.out.println("Animals from file are:");
		for(Animal obj: animalListLoaded)
			System.out.println(obj.getName());
		
		EmployeeRepository emplRepository=new EmployeeRepository("Employees.xml",Constants.XML_TAGS.EMPLOYEE);
		EmployeeRepository emplRepository1=new EmployeeRepository("Employees.xml",Constants.XML_TAGS.EMPLOYEE);
		ArrayList<Employee> e = new ArrayList<Employee>();
		ArrayList<Employee> eLoaded = new ArrayList<Employee>();
		e.add(caretakers[0]);
		e.add(caretakers[1]);
		e.add(caretakers[2]);
		e.add(caretakers[3]);
		emplRepository.save(e);
		/*eLoaded=emplRepository1.load();
		System.out.println("Caretakers from file are:");
		for(Employee obj:eLoaded)
			System.out.println(obj.getName());*/
	}
}
